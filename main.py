...
def object_drawing(img_d, ref_d=None, scale_d=None, tunnel_d=None, offset=0,
    ...
    if tunnel_d is not None:
        for tt in range(len(tunnel_d)):
            ...
            if draw_number:
                img_d = outlined_text(img_d, str(tt + offset), tunnel_d[tt][0] - np.array([object_size, 0]), v_col[0],
                                      font_size)
    return img_d
...
class ImgData:
    ...
    def object_plot(self, img, offset, col_t, object_size, font_size, draw_number, end_node_draw=True):
        img = object_drawing(img, self.ref_xy, self.scale_xy, self.tunnel, offset, col_t, object_size, font_size,
                             draw_number, end_node_draw)
        return img
    ...
    def image_output(self, img, out_dir, object_size, font_size, text_drawing):
        ...
        if len(self.tunnel) < 1:
            cv2.putText(img, "no tunnel", note_pos2,
                        cv2.FONT_HERSHEY_SIMPLEX, font_size, (255, 0, 0), font_size, cv2.LINE_AA)
        if text_drawing:
            img = object_drawing(img, self.ref_xy, self.scale_xy, self.tunnel, 0, v_col[4], object_size, font_size,
                                 draw_number=text_drawing)
    ...
    def colored_image_output(self, img, tunnel_sequence, out_dir, object_size, font_size, text_drawing):
        ...
        for tt in range(len(self.tunnel)):
            img = outlined_text(img, str(tt), self.tunnel[tt][0] - np.array([object_size, 0]),
                                v_col[5 - tunnel_sequence[tt]], font_size)
    ...
    while ii < num_file:
        ...
        while True:
            ...
            def tunnel_length(event, x, y, flags, param):
                ...
                img = tunnel_draw(img,
                                  ((current_tunnel * zoom[0] - zoom_xy[0]) * zoom[1] - zoom_xy[1]) * zoom[2] - zoom_xy[
                                      2],
                ...
                if event == cv2.EVENT_LBUTTONDOWN:
                    current_tunnel = np.append(current_tunnel, (
                                ((np.array([[x, y]]) + zoom_xy[2]) / zoom[2] + zoom_xy[1]) / zoom[1] + zoom_xy[0]) /
                                               zoom[0], axis=0)
                    current_tunnel = current_tunnel.astype(int)

                if event == cv2.EVENT_RBUTTONDOWN:
                    if len(current_tunnel) > 0:
                        img = object_drawing(img, None, None, [
                            ((current_tunnel * zoom[0] - zoom_xy[0]) * zoom[1] - zoom_xy[1]) * zoom[2] - zoom_xy[2]],
                                             count, v_col[4], object_size * zoom[0] * zoom[1] * zoom[2],
                                             font_size * zoom[0] * zoom[1] * zoom[2], draw_number=False)
                        press('p')
                    else:
                        press('f')
...
def postanalysis(in_dir, out_dir, scale_object_len, contact_threshold, network_out, output_image, object_size,
                 font_size, text_drawing):
    ...
    for i_df in tqdm(range(len(tmanual_output[0]))):
                    ...
                    for tt in check_tunnel:
                        ...
                        if len(list_node_on_tunnel) > 0:
                            ...
                            for nn in range(len(list_node_on_tunnel)):
                                ...
                                net_edge_to.append(
                                    node_name[list_start_or_end[node_temp]][list_node_on_tunnel[node_temp]])
                                if nn > 0:
                                    net_edge_len.append(
                                        list_tunnel_seg_len[tunnel_seg_len_order[nn]] - list_tunnel_seg_len[
                                            tunnel_seg_len_order[nn - 1]])
                                else:
                                    net_edge_len.append(list_tunnel_seg_len[tunnel_seg_len_order[nn]])
                                net_edge_from.append(
                                    node_name[list_start_or_end[node_temp]][list_node_on_tunnel[node_temp]])

                            net_edge_to.append(node_name[1][tt])
                            net_edge_len.append(tunnel_len[tt] - list_tunnel_seg_len[
                                tunnel_seg_len_order[len(list_node_on_tunnel) - 1]])
                        else:
                            net_edge_to.append(node_name[1][tt])
        ...
        if network_out:
            for tt in range(len(net_edge_from)):
                df_net.append(
                    [img_data.serial, img_data.id, img_data.name, net_edge_from[tt], net_edge_to[tt], net_edge_len[tt]])
    ...
    while True:
        ...
        if event is None:
            ...
            if event == 'measurement_start':
                ...
                message = postanalysis(in_dir, out_dir, scale_object_len, contact_threshold, network, output_image,
                                       object_size, font_size, text_drawing)
                sg.popup(message)
